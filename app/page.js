...
export default function Home() {
  ...
  const startSidebarDrag = (e) => {
    ...
    const onMove = (ev) => setSidebarWidth(Math.min(480, Math.max(180, startW + ev.clientX - startX)));
  ...
  const startMiddleDrag = (e) => {
    ...
    const onMove = (ev) => setMiddleWidth(Math.min(400, Math.max(160, startW + ev.clientX - startX)));
    const onUp = () => { window.removeEventListener("mousemove", onMove); window.removeEventListener("mouseup", onUp); };
    window.addEventListener("mousemove", onMove);
    window.addEventListener("mouseup", onUp);
  };
  ...
  useEffect(() => {
    ...
    fetch("/api/repos").then(r => r.json()).then(data => { setRepos(Array.isArray(data) ? data : []); setReposLoading(false); }).catch(() => setReposLoading(false));
  }, [session]);

  useEffect(() => {
    if (!selectedRepo) return;
    setTree([]); setExpandedDirs(new Set()); setSelectedDir(null); setTreeLoading(true);
    const [owner] = selectedRepo.fullName.split("/");
    fetch("/api/tree?owner=" + owner + "&repo=" + selectedRepo.name).then(r => r.json()).then(data => { setTree(Array.isArray(data) ? data : []); setTreeLoading(false); }).catch(() => setTreeLoading(false));
  }, [selectedRepo]);
  ...
  const toggleDir = (path) => {
    setExpandedDirs(prev => { const next = new Set(prev); next.has(path) ? next.delete(path) : next.add(path); return next; });
  };
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({ owner, repo: selectedRepo.name, subdir: selectedDir || null, instructions: instructions.trim() || null }),
  ...
  async function handleCommit() {
    ...
    const [owner] = selectedRepo.fullName.split("/");
    try {
      const res = await fetch("/api/commit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ owner, repo: selectedRepo.name, content: readme, subdir: selectedDir || null }) });
  ...
  if (status === "loading") return <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh"}}><SpinnerIcon /></div>;

  if (!session) {
    return (
      <div style={{minHeight:"100vh",display:"flex",flexDirection:"column"}}>
        <nav style={{height:56,display:"flex",alignItems:"center",padding:"0 2rem",borderBottom:"1px solid var(--border)"}}>
          <span style={{fontSize:"1.15rem",fontWeight:500,color:"var(--text)"}}>readmeify</span>
        </nav>
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"4rem 2rem",textAlign:"center"}}>
          <div style={{display:"inline-block",padding:"5px 12px",borderRadius:20,border:"1px solid var(--border2)",color:"var(--muted)",fontSize:".75rem",marginBottom:"1.8rem"}}>
            README generator
          </div>
          <h1 style={{fontSize:"clamp(2.4rem,5vw,4rem)",fontWeight:600,lineHeight:1.15,marginBottom:"1.1rem",color:"var(--text)",maxWidth:640}}>
          ...
          <p style={{color:"var(--muted)",fontSize:"1rem",maxWidth:460,lineHeight:1.7,marginBottom:"2.5rem"}}>
            Connect GitHub, pick a repo or folder, and get a README generated from your actual code — then commit it directly.
          </p>
          <button onClick={() => signIn("github")} style={{display:"flex",alignItems:"center",gap:10,background:"var(--text)",color:"var(--bg)",border:"none",borderRadius:10,padding:"12px 24px",fontSize:".95rem",fontWeight:500,cursor:"pointer"}}
          ...
          <div style={{display:"flex",gap:0,marginTop:"4rem",borderTop:"1px solid var(--border)",paddingTop:"3rem",maxWidth:600,width:"100%"}}>
            {[["Connect","Sign in with GitHub"],["Pick a repo","Browse folders or pick the whole repo"],["Generate","AI writes a README from real context"],["Commit","Push directly to your repo"]].map(([t,d],i,arr)=>(
              <div key={t} style={{flex:1,textAlign:"center",position:"relative",padding:"0 .5rem"}}>
                ...
                <div style={{width:28,height:28,borderRadius:"50%",border:"1px solid var(--border2)",color:"var(--muted)",display:"grid",placeItems:"center",fontSize:".7rem",margin:"0 auto 10px"}}>{String(i+1).padStart(2,"0")}</div>
  ...
  const DragHandle = ({ onMouseDown }) => (
    <div onMouseDown={onMouseDown} style={{position:"absolute",top:0,right:-3,width:6,height:"100%",cursor:"ew-resize",zIndex:10,display:"flex",alignItems:"center",justifyContent:"center"}}>
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      <nav style={{height:56,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 1.5rem",borderBottom:"1px solid var(--border)",position:"sticky",top:0,background:"rgba(13,17,23,.9)",backdropFilter:"blur(12px)",zIndex:10}}>
        <span style={{fontSize:"1.1rem",fontWeight:500,color:"var(--text)"}}>readmeify</span>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <button onClick={() => signOut()} style={{background:"none",border:"1px solid var(--border)",color:"var(--muted)",padding:"5px 12px",borderRadius:7,fontSize:".78rem",cursor:"pointer"}}>Sign out</button>
        </div>
      </nav>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        {/* REPO SIDEBAR */}
        <div style={{width:sidebarWidth,flexShrink:0,position:"sticky",top:56,height:"calc(100vh - 56px)",alignSelf:"flex-start",borderRight:"1px solid var(--border)",display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>
          <DragHandle onMouseDown={startSidebarDrag} />
          <div style={{padding:"1rem",borderBottom:"1px solid var(--border)"}}>
            <div style={{fontSize:".75rem",fontWeight:500,color:"var(--muted)",marginBottom:10}}>repositories</div>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Filter repos..."
              style={{width:"100%",background:"var(--surface2)",border:"1px solid var(--border)",borderRadius:8,padding:"7px 12px",color:"var(--text)",fontSize:".82rem",outline:"none"}}/>
          </div>
              ...
              return (
                <div key={repo.id} onClick={() => { setSelectedRepo(repo); setReadme(""); setCommitState("idle"); setInstructions(""); }}
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
                    <span style={{fontSize:".84rem",color:active?"var(--accent2)":"var(--text)",fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{repo.name}</span>
        ...
        {selectedRepo && (
          <div style={{width:middleWidth,flexShrink:0,position:"sticky",top:56,height:"calc(100vh - 56px)",alignSelf:"flex-start",borderRight:"1px solid var(--border)",display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>
            <DragHandle onMouseDown={startMiddleDrag} />

            <div style={{flex:1,overflow:"auto",padding:"10px 8px"}}>
              <div style={{fontSize:".75rem",fontWeight:500,color:"var(--muted)",marginBottom:8,padding:"0 4px"}}>scope to folder</div>
              ...
              <div onClick={() => setSelectedDir(null)}
                ...
                <span style={{fontSize:".76rem",color:selectedDir===null?"var(--accent2)":"var(--muted)"}}>/ entire repo</span>
              </div>
              ...
              {treeLoading ? (
                ...
                return (
                  <div key={item.path} onClick={() => { if (isDir) { toggleDir(item.path); setSelectedDir(item.path); } }}
                    ...
                    <span style={{fontSize:".75rem",color:isDir?(isSelected?"var(--accent2)":"var(--text)"):"var(--muted)",fontWeight:isDir?500:400,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                      {name}
                    </span>
                    {isDir && isSelected && (
                      <span style={{marginLeft:"auto",fontSize:".58rem",color:"var(--accent)",background:"rgba(35,134,54,.12)",padding:"1px 4px",borderRadius:3,flexShrink:0}}>scoped</span>
                    )}
                  </div>
                );
              })}
        ...
        {/* RIGHT — output */}
        <div style={{flex:1,display:"flex",flexDirection:"column",height:"calc(100vh - 56px)",overflow:"hidden",position:"sticky",top:56,alignSelf:"flex-start"}}>
          {!selectedRepo ? (
            ...
            <>
              <div style={{padding:"12px 20px",borderBottom:"1px solid var(--border)",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:".9rem",fontWeight:500,marginBottom:2,color:"var(--text)"}}>
                ...
                <button onClick={handleGenerate} disabled={generating}
                  style={{display:"flex",alignItems:"center",gap:8,background:generating?"var(--surface2)":"var(--text)",color:generating?"var(--muted)":"var(--bg)",border:generating?"1px solid var(--border2)":"none",borderRadius:8,padding:"8px 18px",fontSize:".85rem",fontWeight:500,cursor:generating?"not-allowed":"pointer"}}>
              ...
              <div style={{flex:1,overflow:"auto",padding:"24px 28px"}}>
                ...
                {(readme||generating) && (
                  <div style={{maxWidth:view==="raw"?900:780,margin:"0 auto"}}>
                    {view==="raw" ? (
                      <pre style={{fontSize:".76rem",color:"var(--muted)",lineHeight:1.8,whiteSpace:"pre-wrap",wordBreak:"break-word"}}>
